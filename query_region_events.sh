...
TIDB_USER="root"
TIDB_PASSWORD=""
LOG_DIR="/Users/cmz/.tiup/data/UjpS4KV/pd-0/"
...
get_current_table() {
  ...
  mysql -h$TIDB_HOST -P$TIDB_PORT -u$TIDB_USER  \
    -e "SELECT CONCAT(db_name, '.', table_name) FROM information_schema.tikv_region_status WHERE region_id = $region_id;" \
    2>/dev/null | awk 'NR==2'
}
...
search_region_events() {
  ...
  grep -h "region-id=$region_id" $LOG_DIR/pd*.log | \
...
# 主函数
main() {
  echo -e "\n=== 开始追踪Region $REGION_ID 的表归属变化 (最近$HOURS小时) ==="
  
  # 初始表状态
  current_table=$(get_current_table $REGION_ID)
  echo -e "\n[当前Region $REGION_ID 所属表: ${current_table:-unknown}]"
  ...
  done < <(search_region_events $REGION_ID $HOURS)
